# CircuitKing Slices 0 & 1 Implementation Plan
...
**Architecture:** A pnpm monorepo with two packages. `@circuitking/parts` owns the ETL from `.fzp` XML plus the hand-authored safety overlay. `@circuitking/circuit` owns the circuit model, union-find netlist derivation, the rule engine, a resistive DC solver, and build-sequence generation. Everything is pure functions over plain data — no rendering, no agent, no UI in this plan.
...
- **Corpus path** is configured via `CK_FRITZING_PATH`, defaulting to `./vendor/fritzing-parts`.
...
- [ ] **Step 1: Add vendor and build artefacts to .gitignore**
...
1. `docs/superpowers/specs/2026-07-28-circuitking-design.md` — the complete
...
- Mains AC (>48 V) is a hard-refusal rule, not a policy note.
...
gh repo create circuitking --private --source=. --remote=origin --push
```

- [ ] **Step 4: Verify the remote has everything**
...
- [ ] **Step 1: Clone and install toolchain on PopOS**

```bash
git clone git@github.com:<user>/circuitking.git && cd circuitking
...
- [ ] **Step 2: Vendor the parts corpus as a submodule**
...
- [ ] **Step 3: Write the environment verification script**
...
echo "corpus:"
corpus="${CK_FRITZING_PATH:-./vendor/fritzing-parts}"
...
describe('scaffold', () => {
  it('exports a package identity', () => {
    expect(PACKAGE_NAME).toBe('@circuitking/parts');
  });
});
```
...
- [ ] **Step 4: Create both packages**

`packages/parts/package.json`:

```json
{
  "name": "@circuitking/parts",
  "version": "0.0.0",
  "type": "module",
  "main": "./dist/index.js",
...
```json
{
  "name": "@circuitking/circuit",
  "version": "0.0.0",
  "type": "module",
  "main": "./dist/index.js",
  "types": "./dist/index.d.ts",
  "dependencies": { "@circuitking/parts": "workspace:*" }
}
```
...
```typescript
export const PACKAGE_NAME = '@circuitking/parts';
```

`packages/circuit/src/index.ts`:

```typescript
export const PACKAGE_NAME = '@circuitking/circuit';
```
...
- Produces:
  - `corpusRoot(): string` — reads `CK_FRITZING_PATH`, defaults to `./vendor/fritzing-parts`
...
export function corpusRoot(): string {
  return resolve(process.env.CK_FRITZING_PATH ?? './vendor/fritzing-parts');
}
...
export function profilesDir(): string {
  return resolve(process.env.CK_PROFILES_PATH ?? './data/profiles');
}
...
- [ ] **Step 4: Author the three seed profiles**
...
- [ ] **Step 6: Generate and inspect the grid**
...
- [ ] **Step 4: Seed the curated manifest**
...
- [ ] **Step 6: Run the whole parts suite**
...
**Interfaces:**
- Consumes: `HoleGrid`, `PartDefinition`, `SafetyProfile` from `@circuitking/parts`
- Produces:
...
import { join } from 'node:path';
import { describe, expect, it } from 'vitest';
import { corpusRoot, extractHoleGrid, loadPart } from '@circuitking/parts';
...
```typescript
import type { HoleGrid, PartDefinition } from '@circuitking/parts';
...
```typescript
import type { Footprint } from '@circuitking/parts';
...
**Semantics:** `split` means pins the student intended to connect ended up on different nodes. `merged` means pins from different intent nets ended up on one node — the shape a rail short takes.
...
```typescript
import type { PartDefinition, SafetyProfile } from '@circuitking/parts';
import type { Board } from '../board.js';
import type { Circuit, PinRef } from '../model.js';
...
describe('polarityRule', () => {
  ...
  it('names the reversed part and tells the student to flip it', () => {
    const ctx = fixtureContext({
      parts,
      nets: [
        net('g', [{ ref: 'U1', pin: 'GND' }, { ref: 'LED1', pin: 'anode' }]),
...
import type { SafetyProfile } from '@circuitking/parts';
...
**The rule:** any net above 48 V, or any part whose profile declares `hazardClass: 'mains'`, produces a `REFUSE`. Per the spec, mains is out of scope by design and the tool declines and explains rather than attempting a partial answer.
...
describe('envelopeRule', () => {
  ...
  it('states the ceiling and leads with what it can still do', () => {
    const ctx = fixtureContext({
      parts: [{ ref: 'PS1', defId: 'mains' }],
      defs: [['mains', MAINS]],
      profiles: [['mains', MAINS_PROFILE]],
    ...
    expect(f.message).toMatch(new RegExp(String(MAX_SAFE_VOLTAGE)));
    expect(f.suggestedFix).toMatch(/low-voltage|isolated|can still/i);
  });
...
const REFUSAL_FIX =
  'I can still help with the low-voltage side of this project — the logic, ' +
  'sensors, and control circuitry — and with sizing an isolated, ready-made ' +
  'power supply module. I will not design or verify anything on the mains side.';
...
export const envelopeRule: Rule = {
  ...
  check(ctx: RuleContext): Finding[] {
    ...
    for (const part of ctx.circuit.parts) {
      if (profileFor(ctx, part.ref)?.hazardClass !== 'mains') continue;
      if (flagged.has(part.ref)) continue;
      flagged.add(part.ref);
      out.push({
        ruleId: 'RULE_OUT_OF_SAFE_ENVELOPE',
        severity: 'REFUSE',
        message:
          `${part.ref} carries mains voltage. This tool works below ` +
          `${MAX_SAFE_VOLTAGE} V only, and will not design or check mains wiring — ` +
          'being almost right about mains is more dangerous than being no help at all.',
        affected: { parts: [part.ref] },
        suggestedFix: REFUSAL_FIX,
      });
    }
    ...
    for (const net of ctx.nets) {
      ...
      out.push({
        ...
        severity: 'REFUSE',
        message:
          `Net "${net.id}" runs at ${v} V, above the ${MAX_SAFE_VOLTAGE} V ceiling ` +
          'this tool works within.',
        affected: { nets: [net.id] },
        suggestedFix: REFUSAL_FIX,
      });
    }
...
**Deliberate scope:** this is *not* SPICE. It is resistive DC with a fixed LED forward-drop, existing only to produce the numbers the gate asks the student to confirm — expected total draw and expected rail continuity. Parts it cannot model contribute their declared `quiescentMa` and are otherwise black boxes.
 ...
 * it exists to produce the numbers the power-up gate asks the student to
...
describe('checkSubstitution', () => {
  ...
  it('reports the current change so the student sees the trade', () => {
...
**This is the product's central claim expressed as tests.** Each entry is a circuit that must always be caught. A regression here is not a failing test — it is a student's destroyed board.
...
const DANGERS: Danger[] = [
  ...
  {
    name: 'mains-rated part anywhere in the circuit',
...
- [ ] **Step 2: Run it — several will fail if any rule regressed**
...
- [ ] **Step 3: Export the package surface**
...
**Both are WARNINGs, not blockers:** an overrun resistor gets hot and drifts but rarely destroys the board, and breadboard rail current is a rule of thumb rather than a hard limit. Marking either as a blocker would train students to dismiss blockers.
...
- [ ] **Step 6: Re-run the danger corpus**
