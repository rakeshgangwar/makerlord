...
| **Schematic layout + symbols** | The current renderer is a deterministic grid of labelled boxes — correct as a projection, weak as a schematic. Interactivity (zoom/pan/hover) landed in the UI 2026-07-29; the layout itself is the gap. | Real component glyphs (resistor/LED/battery/IC) drawn per family, pins on symbol edges, and a proper layered layout — `elkjs` (the engine behind netlistsvg) is the credible dependency. D27 says one layout engine shared with the KiCad generator, so this is a decision entry, not just a patch. | 2–3 days + a D-entry |
