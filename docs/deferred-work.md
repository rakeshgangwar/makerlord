...
| **Curated library → ~150 parts** | **The schedule risk, named since HANDOFF day one.** ~12 fields per part verified against datasheets. **At 22 parts as of 2026-07-30** (LiPo cell + USB charger joined; 8 carry D25 KiCad mappings) (caps, 1N4001 + SPICE model, pushbutton, pot, motor, relay, RGB LED, LDR, DHT22, batteries, TO-220 NPN, servo, WeMos D1 mini, soil-moisture sensor, LD1117V33 regulator) — footprints extracted from corpus SVG geometry, datasheet values cited in each profile, `curated-manifest.test.ts` is the permanent gate. | The D50/D51 pipeline (2026-07-30) changes the economics: the agent drafts proposals with citations (`profile_propose`), `maker curate` promotes after human review, the whole corpus browses as geometry tier. Remaining drip: ESP32-class board, more sensors, MOSFETs; SPICE models + more KiCad mappings (8 done). | Demand-driven |
| **Four deferred rules** (flyback, source capacity, LiPo, decoupling) ✅ | **All four shipped.** 2026-07-29: `RULE_FLYBACK_MISSING` (BLOCKER), `RULE_SOURCE_OVER_CAPACITY` (WARNING), `RULE_DECOUPLING_MISSING` (WARNING). 2026-07-30: `RULE_LIPO_UNMANAGED` (BLOCKER — a hazardClass 'lipo' cell reaching anything not hand-marked `managesLipo`), unlocked by the curated SparkFun LIPO-1000mAh cell + USB LiPoly Charger (MCP73831T), with its Tier-1 danger-corpus row. Twelve rules total. | — | done |
